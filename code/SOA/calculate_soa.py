...
from SOA.darknet import Darknet
from SOA.dataset import YoloDataset
from SOA.util import *
...
def run_yolo(args):
    ...
    classes = load_classes('SOA/data/coco.names')
...
def calc_soa(args):
    ...
    soa_values.append("Class average accuracy for all classes (SOA-C) is: {:6.4f}".format(class_average_acc))

    image_average_acc = calc_image_weighted_average_accuracy(results_dict)
    soa_values.append("Image weighted average accuracy (SOA-I) is: {:6.4f}".format(image_average_acc))

    top_40_class_average_acc, bottom_40_class_average_acc = calc_split_class_average_accuracy(results_dict)
    soa_values.append("Top (SOA-C-Top40) and Bottom (SOA-C-Bot40) 40 class average accuracy is: {:6.4f} and {:6.4f}".
