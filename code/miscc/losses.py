...
def discriminator_loss(netD, real_imgs, fake_imgs, conditions,
    ...
    return total_errD, disc_dist_lambda * disc_losses.detach() if type(disc_losses) != int else disc_losses, \
            errD.detach().item(), err_distil.detach().item() if type(err_distil) != float else err_distil, \
...
def generator_loss(netsD, image_encoder, fake_imgs, real_labels,
    ...
    for i in range(numDs):
        ...
        if netsD[i].UNCOND_DNET is  not None:
            ...
            g_loss = cond_errG
        errG_total += g_loss
        # err_img = errG_total.detach().item()
        logs += 'g_loss%d: %.2f ' % (i, g_loss.detach().item())
        ...
        if i == (numDs - 1):
            ...
            # err_words = err_words + w_loss.detach().item()

            s_loss0, s_loss1 = sent_loss(cnn_code, sent_emb,
                                         match_labels, class_ids, batch_size)
            s_loss = (s_loss0 + s_loss1) * \
                cfg.TRAIN.SMOOTH.LAMBDA
            # err_sent = err_sent + s_loss.detach().item()

            errG_total += w_loss + s_loss
            logs += 'w_loss: %.2f s_loss: %.2f ' % (w_loss.detach().item(), s_loss.detach().item())
